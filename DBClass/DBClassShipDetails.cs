...
{
    class DBClassShipDetails
    {
        public string TableName = "ShipDetails";
        string errorMsg;
        string ConnString;
        ...
        /// <returns></returns>
        public SIS.Configuration.Items[] QueryData(string ShipID)
        {
            InitDB();
            string selectCmd;
                ...
                if (ds.Tables[TableName].Rows.Count >= 1)
                {
                    ...
                    foreach (DataRow row in ds.Tables[TableName].Rows)
                    {
                        if (row["ItemsID"] != null)
                        {
                            ...
                            oneItem.Price = int.Parse(row["SellingPrice"].ToString());
        ...
        public int QueryItemQuantity(string ShipID , string ItemsID)
        {
            InitDB();
            string selectCmd;
            int Quantity = 0;
            selectCmd = "Select Quantity From " + TableName + " Where ShipID='" + ShipID + "' And ItemsID='" + ItemsID +"'";
